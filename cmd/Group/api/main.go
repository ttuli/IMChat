...
import (
	...
	"IM2/internal/apps/Group/api/handler"
...
var configPath = flag.String("f",
	configparser.DefaultConfigPath("Group/api"),
	"the config file")
...
func main() {
	...
	runner := service.NewServiceRunner(
		...
		*configPath,
		&config.Config{},
		service.WithName("Group API Service"),
		service.WithLogger("/var/log/im/group.api.log", logger.LoggerEnvDev),
	)
