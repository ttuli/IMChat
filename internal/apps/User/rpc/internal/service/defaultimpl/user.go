...
func (s *userService) CreateUser(ctx context.Context, info *model.UserInfo) (uint64, error) {
	...
	if err != nil {
		return 0, xerr.Wrap(err, xerr.ErrRPC, "调用ID生成服务失败")
	}
	if len(resp.Ids) == 0 {
		return 0, xerr.New(xerr.ErrDatabase, "生成ID失败")
	}
	info.UserID = uint64(resp.Ids[0])
	err = s.userDAO.InsertUser(ctx, info)
	if err != nil {
		return 0, xerr.Wrap(err, xerr.ErrDatabase, "插入用户失败")
	}
...
func (s *userService) VerifyPassword(ctx context.Context, userId uint64, password string) (bool, error) {
	...
	if err == gorm.ErrRecordNotFound {
		return false, xerr.New(xerr.ErrNotFound, "用户不存在")
	}
	if err != nil {
		return false, xerr.Wrap(err, xerr.ErrDatabase, "查询用户失败")
	}
...
func (s *userService) GetUsersByIDs(ctx context.Context, ids []uint64) ([]*model.UserInfo, error) {
	...
	if err != nil {
		return nil, xerr.Wrap(err, xerr.ErrDatabase, "批量查询用户失败")
	}
	return users, nil
}
...
func (s *userService) GetUserByPhone(ctx context.Context, phone string) (*model.UserInfo, error) {
	...
	if err == gorm.ErrRecordNotFound {
		return nil, xerr.New(xerr.ErrNotFound, "用户不存在")
	}
	if err != nil {
		return nil, xerr.Wrap(err, xerr.ErrDatabase, "查询用户失败")
	}
	return user, nil
}
...
func (s *userService) GetUsersByName(ctx context.Context, name string, limit, offset int32) ([]*model.UserInfo, error) {
	...
	if err != nil {
		return nil, xerr.Wrap(err, xerr.ErrDatabase, "按名字查询用户失败")
	}
	return users, nil
}
...
func (s *userService) UpdateUserInfo(ctx context.Context, id uint64, name, avatar string, gender, joinType uint8, personalSignature string) error {
	...
	if err == gorm.ErrRecordNotFound {
		return xerr.New(xerr.ErrNotFound, "用户不存在")
	}
	if err != nil {
		return xerr.Wrap(err, xerr.ErrDatabase, "查询用户失败")
	}
	...
	if err != nil {
		return xerr.Wrap(err, xerr.ErrDatabase, "更新用户失败")
	}
	return nil
}
